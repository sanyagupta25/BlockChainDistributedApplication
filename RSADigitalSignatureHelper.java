...
static void sendMessage(BufferedReader bR,Client client) throws IOException{ 
	...
	IntStream.range(0,m.length).forEach(i-> x.append(RSADigitalSignatueHelper.characterToAscii(m.chatAt(i)))+ "  "); 
		System.out.println("["+client.getName()+"]: map char to ascii message ==> x="+x); 
		boolean flag=true; 
		while(flag)
		{                             
			try { 
				System.out.println("[system]: please enter keyy to sign message with"); 
				...
				if(client.getOtherPartyUsername()!=null) {
					System.out.print("["+client.getName()+"]: send 'signed transaction' = (transaction,signature) ==> (x,s) = (' "+x.toString()+" ',' ");
							System.out.println(xPrimeSB.toString().trim()+")")); 
					StringWriter sW=new StringWriter(); 
					Json.createWriter(sW).writeObject(Json.createObjectBuilder().add("name", cleint.getName())
							.add("x",x.toString())
...
{ 
	...
	else System.out.println("["+client.getName()+"]: (2)compare message to calculated message: x!=x' ==> message was not signed w/"+otherPartyName+ " 's private key");
}
